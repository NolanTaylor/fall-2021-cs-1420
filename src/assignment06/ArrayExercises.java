...
public class ArrayExercises
{
    ...
    {
    	char[] tempArray = new char[symbols.length];
    	
    	// create a copy of symbols but it needs to be a separate object
    	// (not just two variables pointing to the same object)
    	for (int i = 0; i < symbols.length; i++)
    	{
    		tempArray[i] = symbols[i];
    	}
    	
    	// for each item in tempArray copy the reverse position into symbols
    	for (int i = 0; i < symbols.length; i++)
    	{
    		symbols[i] = tempArray[symbols.length - 1 - i];
    	}
    ...
    public static int count (Object[] values, Object target)
    {
    	...
    	// for each value in the array, if the value equals the target increment
    	// the count by one. then return the count
    ...
    {
    	// for each item in the list. if the item is the original, replace it with the replacement
    ...
    public static double[] computeAreas (double[] widths, double[] heights)
    {
    	...
    	// loop through the range of the array and set the corresponding index of the
    	// areas array to the area of the rectangle
    ...
    {
    	int newArraySize = pixels.length;
    	
    	// calculate the size of the new array by subtracting each
    	// instance of the target color from an integer
    	...
    	int filled = 0;
    	Color[] newArray = new Color[newArraySize];
    	
    	// for each color in pixels, add it to the new array and increment the count
    	// only if it isn't the target color
		for (Color pixel : pixels)
		{
			if (pixel == null && target == null) { }
			else if (pixel == null && target != null)
			{
				newArray[filled] = null;
				filled++;
			}
			else if (!pixel.equals(target))
			{
				newArray[filled] = pixel;
				filled++;
			}
		}
    ...
    {
    	// for each value find the largest value ahead of it in the array
    	for (int i = 0; i < data.length; i++)
    	{
    		...
    		// then swap the current index with the largest value
    		int temp1 = data[i];
    		int temp2 = data[largestIndex];
    		
    		data[i] = temp2;
    		data[largestIndex] = temp1;
    	}
    ...
    public static Rectangle findSmallest (Rectangle[] rectangles)
    {
    	...
    	{
    		if (rect.width * rect.height <= smallestRect.width * smallestRect.height)
    		{
    			smallestRect = rect;
    		}
    	}
